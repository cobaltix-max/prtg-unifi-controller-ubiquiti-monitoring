...
#Query API providing token from first query for device stats
try {
...
#Query API providing token from first query for site health
try {
$jsonresulthealth = Invoke-Restmethod -Uri "$controller/proxy/network/api/s/$site/stat/health/" -WebSession $myWebSession
}catch{
	Write-Output "<prtg>"
	Write-Output "<error>1</error>"
	Write-Output "<text>API Query Failed: $($_.Exception.Message)</text>"
...
Foreach ($subsystem in $jsonresulthealth.data)
{
    if($subsystem.status -like "ok"){$val = 0} Else {$val = 1}
    Write-Host "<result>"
    Write-Host "<channel>$($subsystem.subsystem.ToUpper()) Status</channel>"
...
Pause
Write-Host "<result>"
Write-Host "<channel>Gateway Status</channel>"
Write-Host "<value>$($var)</value>"
Write-Host "</result>"

Write-Host "<result>"
Write-Host "<channel>Switch Status</channel>"
Write-Host "<value>$($var)</value>"
Write-Host "</result>"

Write-Host "<result>"
Write-Host "<channel>Wireless Status</channel>"
Write-Host "<value>$($var)</value>"
Write-Host "</result>"

Write-Host "<result>"
Write-Host "<channel>VPN Status</channel>"
Write-Host "<value>$($var)</value>"
Write-Host "</result>"

Write-Host "<result>"
Write-Host "<channel>Controller Status</channel>"
Write-Host "<value>$($var)</value>"
Write-Host "</result>"
#>
...
if ($debug){
	[string]$logPath = 'C:\temp' #((Get-ItemProperty -Path "hklm:SOFTWARE\Wow6432Node\Paessler\PRTG Network Monitor\Server\Core" -Name "Datapath").DataPath + "Logs (Sensors)\"
